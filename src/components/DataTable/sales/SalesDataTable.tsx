...
import { TRANSLATE_PAYMENT_METHODS, keyTPaymentMethod } from '@/constants/translations/payments.methods';
...
const columns: ColumnDef<Sale>[] = [
  ...
  { accessorKey: "amount", header: "Monto", cell: ({ row }) => `${formatCurrency(row.getValue('amount'))}` },
  { accessorKey: "profits", header: "Ganancias", cell: ({ row }) => `${row.getValue('profits')?.toLocaleString()}` },
  { 
    accessorKey: "products",
    header: "Productos",
    cell: ({ row }) => row.original.products.reduce((acc, curr) => acc + curr.quantity, 0) + " unidades"
  },
  { accessorKey: "paymentMethod", header: "Método de Pago", 
    cell: ({row}) => `${TRANSLATE_PAYMENT_METHODS[row.getValue("paymentMethod") as keyTPaymentMethod]}`
   },
  { 
    accessorKey: "bills",
    ...
    cell: ({ row }) => {
      ...
      return Object.entries(bills).map(([denom, count]) => `${count}x${formatCurrency(Number(denom))}`).join(', ')
    }
  },
  { 
    ...
    cell: ({ row }) => {
      ...
      return Object.entries(cashBack).map(([denom, count]) => `${count}x$${denom}`).join(', ');
    }
  },
  {
...
// Component
export default function SalesDataTable({ date = new Date() }: Props) {
