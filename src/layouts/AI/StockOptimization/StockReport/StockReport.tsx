...
export default function StockReport({ startDate, endDate }: Props) {
  ...
  if (!data) {
    return (
      <Alert variant="destructive">
        ...
        <AlertDescription>No se pudieron cargar los datos de optimización de stock.</AlertDescription>
      </Alert>
    );
  }
  ...
  const totalDemand = data.predictions.reduce((sum, item) => sum + item.DemandaPronosticada, 0);
  const totalRecommendedStock = data.predictions.reduce((sum, item) => sum + item.StockRecomendado, 0);
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Análisis del período: {new Date(startDate).toLocaleDateString()} - {new Date(endDate).toLocaleDateString()}
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-medium">Demanda Total Pronosticada</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="text-2xl font-bold">{totalDemand.toFixed(1)} unidades</div>
              </CardContent>
            </Card>
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-medium">Stock Total Recomendado</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="text-2xl font-bold">{totalRecommendedStock} unidades</div>
              </CardContent>
            </Card>
          </div>
        </CardContent>
      ...
      <Tabs defaultValue="table" className="w-full">
        ...
        <TabsContent value="table" className="mt-6">
          <Card>
            ...
            <CardContent>
              <Table>
                <TableCaption>Predicciones para el próximo período</TableCaption>
                <TableHeader>
                  <TableRow>
                    <TableHead>Producto</TableHead>
                    <TableHead className="text-right">Demanda Pronosticada</TableHead>
                    <TableHead className="text-right">Stock de Seguridad</TableHead>
                    <TableHead className="text-right">Stock Recomendado</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {data.predictions.map((prediction, index) => (
                    <TableRow key={index}>
                      <TableCell className="font-medium">{prediction.Producto}</TableCell>
                      <TableCell className="text-right">{prediction.DemandaPronosticada.toFixed(1)}</TableCell>
                      <TableCell className="text-right">{prediction.StockSeguridad.toFixed(1)}</TableCell>
                      <TableCell className="text-right font-bold">{prediction.StockRecomendado}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
              ...
              <CardContent className="h-96">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    ...
                    <Tooltip formatter={(value) => [`${value} unidades`, 'Demanda Pronosticada']} />
                  </PieChart>
                </ResponsiveContainer>
              </CardContent>
            </Card>
